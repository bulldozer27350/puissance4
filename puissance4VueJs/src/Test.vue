<template>
  ...
  <div v-if="initialization" id="initialization" class="player-inputs">
    ...
    <br/>
    <button @click="initializeGame">Commencer une partie</button>
  </div>

  <div v-if="!initialization && !initialized">
...
mounted() {
  ...
  const socket = io('http://localhost:4444'); // Assurez-vous que l'URL correspond à votre serveur Socket.io

  socket.on(`/topic/game/${idPartie}`, (message) => {
    // Mettez à jour l'état du jeu en fonction du message reçu
    const updatedGame = JSON.parse(message);
    // Mettez à jour votre modèle de vue (state) en fonction de la réponse du serveur
    // ...
  });
},
methods: {
